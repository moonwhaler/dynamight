<script lang="ts">
  ...
  import { getFileIcon, formatFileSize, formatFileType, formatDate } from '$lib/utils/fileIcons';
  ...
  let {
    ...
    selected = false,
    onSelect,
    visibleColumns = ['name', 'type', 'size', 'modified', 'actions'],
    searchQuery = '',
...
{#if viewMode === 'list'}
  ...
  >
    {#each visibleColumns as col (col)}
      ...
      {:else if col === 'type'}
        <td class="px-4 py-2.5 text-gray-500 dark:text-gray-400 text-sm truncate">
          {formatFileType(entry.extension, entry.is_dir)}
        </td>
      {:else if col === 'size'}
